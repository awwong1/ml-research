...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=1000, classifier_input_features=512, init_weights=True):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(classifier_input_features * 7 * 7, 4096),
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(4096, 4096),
            nn.ReLU(True),
    ...
    for v in cfg:
        if v == "M":
            ...
            if sparsity_mask:
                mask_size = (1, v, 1, 1)
                mask = MaskSTE(
                    mask_size, kernel_size=conv2d.kernel_size, in_channels=in_channels
                )
                layers += [mask]
            in_channels = v
...
def vgg_from_layers(layers=[], batch_norm=True, **kwargs):
    ...
    model = VGG(
        make_layers(layers, batch_norm=batch_norm, sparsity_mask=False), **kwargs
    )
    return model
